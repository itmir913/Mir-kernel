...
static int yaffs_ScanBackwards(yaffs_Device *dev)
{
	...
	__u32 sequenceNumber;
	yaffs_ObjectHeader *oh;
#ifdef FEATURE_SKY_YAFFS2_PLC_SUPPLEMENT
	yaffs_ObjectHeader *sky_oh;
#endif
	yaffs_Object *in;
	yaffs_Object *parent;
	int nBlocks = dev->internalEndBlock - dev->internalStartBlock + 1;
			...
			if (!tags.chunkUsed) {
...
#ifdef FEATURE_SKY_YAFFS2_PLC_SUPPLEMENT
					if(tags.chunkId > YAFFS_MAX_CHUNK_ID)
					{
						oh = NULL;		
						oh = (yaffs_ObjectHeader *) chunkData; 

						tags.chunkId &= YAFFS_MAX_CHUNK_ID;
						printk(KERN_INFO "[### JYKWON ### tags.chunID MAX OVER(File NAME :: %s)]",oh->name);
					}
#endif
...
#ifdef FEATURE_SKY_YAFFS2_PLC_SUPPLEMENT
				{
					...
					T(YAFFS_TRACE_ERROR, (
					    ...
					    chunk));
#ifdef FEATURE_SKY_YAFFS2_PLC_SUPPLEMENT
/* coded by PZ1643@PS2(revival) / 2011/04/14 / YAFFS2 Exception process */
					if(YAFFS_OBJECT_TYPE_SPECIAL >= (oh ? oh->type : tags.extraObjectType) ) 
/* end of PZ1643@PS2(revival) */
					{
						sky_oh = NULL;		
						sky_oh = (yaffs_ObjectHeader *) chunkData; 

						yaffs_DeleteObject(in);
if(parent)
						yaffs_DeleteObject(parent);
						yaffs_DeleteChunk(dev, chunk, 1, __LINE__);

						yaffs_DoGenericObjectDeletion(in);
if(parent)
						yaffs_DoGenericObjectDeletion(parent);
						yaffs_UpdateObjectHeader(in, NULL, 1,  0, 0);						
if(parent)
						yaffs_UpdateObjectHeader(parent, NULL, 1,  0, 0);						
						return YAFFS_SKY;
					}
#endif
				}
	...
	if (!init_failed) {
		...
		if (dev->isYaffs2) {
			if (yaffs_CheckpointRestore(dev)) {
				...
				if (!init_failed )
				{
					int result_ScanBackwards;

					result_ScanBackwards = yaffs_ScanBackwards(dev);
					
					if(!result_ScanBackwards)
						init_failed = 1;
					else if(result_ScanBackwards == YAFFS_SKY)
					{
