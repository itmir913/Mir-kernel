...
extern void pantech_errlog_init_log(void);
extern void pantech_errlog_add_stack(const char *name, int size);

static int isWarnLog = 0;	// Do not save warning log.
#endif
...
void dump_backtrace_entry(unsigned long where, unsigned long from, unsigned long frame)
{
	...
	if(!isWarnLog)
	{
		...
		if(buffer[0] != 0x00)
		{
			pantech_errlog_add_stack(buffer, strlen(buffer));
		}
	}
#endif
...
#ifdef CONFIG_PANTECH_ERROR_LOG
/// wjmin20100924@DS2 changed the order
  pantech_errlog_display_with_errlog(false, false);
  pantech_errlog_noti_crash_to_marm(SYS_RESET_REASON_LINUX);
#endif
