...
// dscheon 2010.5.10@DS2 for root shell
		uint32_t	dmterminal:1;	// root shell on/off
		uint32_t	fakeusb:1;	// fake usb for data cable (tethering debug)
		uint32_t	consusdis:1;	// disable console suspend
		uint32_t	reserved:8;
		uint32_t	useless:16;
		// MSB
	} bits;
...
#if defined(CONFIG_UART2DM_CONSOLE)
// wjmin 2010.5.4@DS2 for root shell
extern void msm_hs_console_set_terminal_onoff(uint32_t onoff);
#elif defined(CONFIG_SERIAL_MSM_HSL_CONSOLE)
// wjmin 2010.08.18@DS2 for root shell
extern void msm_hsl_console_set_terminal_onoff(uint32_t onoff);
#endif // #if defined(CONFIG_UART2DM_CONSOLE)
#if 0 /* EF31 - ginger break */
extern void msm_batt_sky_fake_usb_info(uint32_t onoff);
#endif
...
// wjmin 2010.5.4@DS2 for root shell
	if (debug_flag.bits.dmterminal) {
		...
		msm_hs_console_set_terminal_onoff(1);
	}
	else {
		printk(KERN_INFO "%s: UART ROOT SHELL OFF\n", __func__);
		msm_hs_console_set_terminal_onoff(0);
	}
#elif defined(CONFIG_SERIAL_MSM_HSL_CONSOLE)
// wjmin 2010.08.18@DS2 for root shell
	if (debug_flag.bits.dmterminal) {
...
#endif // #ifdef CONFIG_UART2DM_CONSOLE

	// added by jwcha100803@DS2 for tethering debug
	if (debug_flag.bits.fakeusb) {
		printk(KERN_INFO "%s: FAKE USB ON\n", __func__);
#if 0 /* EF31 - ginger break */
		msm_batt_sky_fake_usb_info(1);
#endif
	}
	else {
		printk(KERN_INFO "%s: FAKE USB OFF\n", __func__);
#if 0 /* EF31 - ginger break */
		msm_batt_sky_fake_usb_info(0);
#endif
	}

#if	0
	// NOT using in booting time by jwcha100806@DS2
	// added by jwcha100806@DS2 for suspending console
	if (debug_flag.bits.consusdis) {
		printk(KERN_INFO "%s: console suspend is disabed\n", __func__);
		printk_console_suspend_disable(1);
	}
	else {
		printk(KERN_INFO "%s: console suspend is enabled\n", __func__);
		printk_console_suspend_disable(0);
	}
#endif
}
