...
#if defined(CONFIG_PANTECH_ERROR_LOG)
    pantech_errlog_display_add_log(x, strlen(x));
#endif
...
{
#if defined(CONFIG_PANTECH_ERROR_LOG)
  pantech_errlog_display_add_log("ARM9 has crashed", strlen("ARM9 has crashed"));
#endif
	pr_err("MODEM/AMSS has CRASHED\n");
	smd_diag();
#if defined(CONFIG_PANTECH_ERROR_LOG)
//  pantech_errlog_display_with_errlog(true, true);
...
#if defined(CONFIG_PANTECH_ERROR_LOG)
    val = readl(SMSM_STATE_ADDR(SMSM_MODEM_STATE));
    if (val & SMSM_RESET) {
      val = val & ~SMSM_RESET;
      handle_modem_crash();
      return -1;
