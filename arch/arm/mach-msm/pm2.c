...
#ifdef SLEEP_LOG_EVENT_AFTER_STATE_3		// Inserted by dscheon 
int SleepSetEventForPMLOG = 0;

int GetSleepEvent(void)
{
	return SleepSetEventForPMLOG;
}
#endif
...
// by jwcha100812@DS2 for wakeup reason
#define DEM_WAKEUP_REASON_NONE       0x00000000
#define DEM_WAKEUP_REASON_SMD        0x00000001	// RPC
...
char * get_wakeup_reason_string(unsigned int reason)
{
	switch (reason) {
...
#ifdef	CONFIG_EF31S_32K_BOARD
	// by jwcha100812@DS2
	printk(KERN_ERR "%s : wakeup_reason (0x%x:%s:%d)\n", __func__, 
				msm_pm_smem_data->wakeup_reason,
				get_wakeup_reason_string(msm_pm_smem_data->wakeup_reason),
				msm_pm_smem_data->gpioInfo);
#endif	// CONFIG_EF31S_32K_BOARD
...
void arch_idle(void)
{
	...
	int ret;
	int i;
#ifdef SLEEP_LOG_EVENT_AFTER_STATE_3
	int Isallowed = 0;
#endif
#ifdef CONFIG_MSM_IDLE_STATS
...
#ifndef  SLEEP_LOG_EVENT_AFTER_STATE_3
	...
	if ((timer_expiration < msm_pm_idle_sleep_min_time))
	{
		if (GetSleepEvent())
		{
			printk(KERN_ERR "[PM_DEBUG]: Timer \n");
		}
		Isallowed = 1;
	}	
	if (has_wake_lock(WAKE_LOCK_IDLE))
	{
		if (GetSleepEvent())
		{
			printk(KERN_ERR "[PM_DEBUG]: WAKE_LOCK_IDLE \n");
		}
		Isallowed = 1;
	}
	if (!msm_irq_idle_sleep_allowed())
	{
#if (1)	
		if (GetSleepEvent())
		{
			printk(KERN_ERR "[PM_DEBUG]: IRQ \n");
		}
#endif
		Isallowed =1;
	}
	...
	if (GetSleepEvent())
	{
		printk(KERN_ERR "[CDS]: next_timer %lld, sleep limit %u \n",timer_expiration, sleep_limit);
	}
#endif
...
{
#if 1	//Bug Fix - 2011.02.23 (Ginger Bread Reboot Sync...(Pantech - PS2))
	mdelay(10);
#endif
...
static void msm_pm_restart(char str, const char *cmd)
{
	...
	msm_rpcrouter_close();
#if defined(CONFIG_SW_RESET)
	if(restart_reason == SYS_RESET_REASON_ANDROID || restart_reason == SYS_RESET_REASON_LINUX){
/// wjmin20100924@DS2 changed
    sky_sys_rst_NotiToMARM(&restart_reason);
//		sky_sys_rst_SwReset(&restart_reason);
	} else {
		sky_sys_rst_UserReset(&restart_reason);
	}
#else
...
/// Added by wjmin 2010.3.28
#ifdef CONFIG_EF31S_32K_BOARD
		} else if (!strcmp(cmd, "PDL") || !strcmp(cmd, "pdl")) {
			restart_reason = 0x6f656d66;
/// wjmin2010.06.25@DS2 Added 
#ifdef CONFIG_SW_RESET
		} else if (!strcmp(cmd, "androidpanic")) {
			restart_reason = SYS_RESET_REASON_ANDROID;
#endif
#endif
#if 1//defined(FEATURE_SKY_SDCARD_UPGRADE)		//pantech_lks 20110315 for test //self_upgrade_patch_PS1_110321
        } else if (!strncmp(cmd, "update", 6)) {
          printk("........... msm_reboot_call  : update \n");

          restart_reason = 0xA5A5A5A5;//SYS_RESET_SD_IMAGE_UPDATE_AARM_COMPLETED;
#endif
		} else {
...
/// wjmin2010.06.25@DS2 Added 
  ...
  uint32_t reset_reason = SYS_RESET_REASON_UNKNOWN;
#endif
#ifdef CONFIG_CPU_V7
	pgd_t *pc_pgd;
	pmd_t *pmd;
...
/// wjmin2010.06.25@DS2 Added 
	...
	reset_info = create_proc_entry("pantech_resetinfo", S_IRUGO | S_IWUGO, NULL);
	...
	// set default value in mARM boot. but, re-set in aARM
	sky_sys_rst_SetSwReset(&reset_reason);	
#endif
