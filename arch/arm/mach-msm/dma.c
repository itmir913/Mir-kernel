...
/// by dscheon
#if 1
void msm_dmov_disable_channel(unsigned int id)
{
	unsigned long irq_flags;
//	struct msm_dmov_cmd *cmd;
	...
	dmov_conf[adm].channel_active &= ~(1U << id);
	if (!dmov_conf[adm].channel_active) {
...
EXPORT_SYMBOL(msm_dmov_disable_channel);

void msm_dmov_enable_channel(unsigned int id)
{
	unsigned long irq_flags;
//	struct msm_dmov_cmd *cmd;
...
EXPORT_SYMBOL(msm_dmov_enable_channel);
#endif
