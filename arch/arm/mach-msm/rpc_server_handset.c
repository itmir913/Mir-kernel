...
#ifdef CONFIG_EF31S_32K_BOARD //by elecjang 20100303 for Headset key
// Inserted by DSCHEON for Headset detect. 2009.08.20
#include "proc_comm.h"
#include "qdsp5/sky_snd_ext_amp_max8981.h" //"qdsp5/sky_snd_ext_amp_max97001.h" 97001 -> 8981 amp test 101214
#endif

// novapex_hsj_20110509__power_wakeup_wait_1sec
#define POWER_WAKEUP_WAIT_1SEC
...
#ifdef FEATURE_SKY_SND_MODIFICATION
#define HS_STEREO_HEADPHONE_K   0xB4
#endif
#if 1 //def FEATURE_SKY_3P5PI_EARJACK_USE_KEY		//sylee
...
static const uint32_t hs_key_map[] = {
	...
	KEY(HS_SKY_3P5PI_STEREO_HEADSET_NO_MIC_K, SW_HEADPHONE_INSERT),
	KEY(HS_SKY_3P5PI_STEREO_HEADSET_K, SW_HEADPHONE_INSERT),
#endif
#ifdef FEATURE_SKY_CHG_LOGO
...
#ifdef CONFIG_EF31S_32K_BOARD//modified by elecjang 20100113, pull down 
// Inserted by DSCHEON for Headset detect. 2009.08.20
void Send2Msm_Handset_Block (int HS_state)
{
	int key;
        switch (HS_state)
        {
                case 0://HEADSET REMOVE
			#if 1 /* EF31 - ginger bread */
			key = SW_HEADPHONE_INSERT;
			hs->hs_on = HS_HEADSET_DISCONNECTED;
			input_report_switch(hs->ipdev, key, hs->hs_on);
			update_state();
			#else
                        report_headset_switch(hs->ipdev, SW_HEADPHONE_INSERT, HS_HEADSET_DISCONNECTED);
                                        //printk(KERN_ERR "PANTECH HEADSET REMOVE\n");
			#endif																				
                        break;
                case 1://HEADSET INSERT
			#if 1 /* EF31 - ginger bread */
			key = SW_HEADPHONE_INSERT;
			hs->hs_on = HS_HEADSET_CONNECTED;
			input_report_switch(hs->ipdev, key, hs->hs_on);
			update_state();
			#else
                        report_headset_switch(hs->ipdev, SW_HEADPHONE_INSERT, HS_HEADSET_CONNECTED);
                                        //printk(KERN_ERR "PANTECH HEADSET INSERT\n");
			#endif																				
                        break;
        }
}
EXPORT_SYMBOL(Send2Msm_Handset_Block);
...
static int current_headset_state=0;
static int ex_headset_state=0;

static ssize_t show_headset(struct device *dev, struct device_attribute *attr, char *buf)
{
       //printk(KERN_ERR "show_headset %d\n", current_headset_state);
	return snprintf(buf, PAGE_SIZE, "%d\n", current_headset_state);
}

static ssize_t set_headset(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
       //printk(KERN_ERR "show_headset %d\n", current_headset_state);
	return 0;
}
...
#ifdef POWER_WAKEUP_WAIT_1SEC
static struct wake_lock power_wae_lock;
#endif
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	printk(KERN_ERR "###### [key : %x][key_code : %x][key_parm : %x]\n",key,key_code,key_parm);
...
#ifdef POWER_WAKEUP_WAIT_1SEC
		if (key == KEY_POWER) {
			pr_info("report_hs_key: resume_wakeup\n");
			wake_unlock(&power_wae_lock);
			wake_lock_timeout(&power_wae_lock, HZ);
		}
#endif
		break;
	...
	case SW_HEADPHONE_INSERT:
#if (1)//FEATURE_SKY_SND_MODIFICATION
#ifdef CONFIG_EF31S_32K_BOARD
		if(key_code!=HS_REL_K)   {
			hs->hs_on = 1;
			if(key_code==HS_STEREO_HEADPHONE_K)
			{
				current_headset_state |= 0x1;
				hs->mic_on = 0;
			}
			else
			{
				current_headset_state |= 0x2;
				hs->mic_on = 1;
			}
		}
		else 
		...
		update_state();
#ifdef FEATURE_BOOT_SOUND_PLAY
		  // SangwonLee 100512 Boot Sound.
		   if(boot_snd_play>=BOOTSND_SET_PATH)	 {
			   if(!ex_headset_state)  { // ex speaker
					if(current_headset_state)	{	// now headset
						snd_extamp_api_SetDevice(1, SND_DEVICE_STEREO_HEADSET);
						snd_extamp_api_SetDefaultVolume(SND_DEVICE_STEREO_HEADSET, 1);
					}
			   }
			   else { // ex headset
					if(!current_headset_state)	{	// now speaker
						snd_extamp_api_SetDevice(1, SND_DEVICE_SPEAKER_PHONE);
						snd_extamp_api_SetDefaultVolume(SND_DEVICE_SPEAKER_PHONE, 1);
					}
			   }
		   }
		   if(current_headset_state)	   earjack_status = 1;
		   else earjack_status = 0;
#endif
#else
		// GINGERBREAD CODE 	
...
#ifdef CONFIG_EF31S_32K_BOARD
               if(key_code==HS_REL_K)   {
                    if(key_parm==HS_STEREO_HEADPHONE_K) current_headset_state &= 0x1;
                    else    current_headset_state &= 0x2;
               }
               else {
                    if(key_code==HS_STEREO_HEADPHONE_K) current_headset_state |= 0x2;
                    else    current_headset_state |= 0x1;
               }
      		//printk(KERN_ERR "[SKY Headset] headset (state:%d)\n", current_headset_state);
              /*if((ex_headset_state>0 && current_headset_state==0) || 
                    (ex_headset_state==0 && current_headset_state>0))   */
#if 1
{
		// GINGERBREAD CODE 	
		hs->hs_on = (key_code != HS_REL_K) ? 1 : 0;
...
#else
			  {
                     //printk(KERN_ERR "[SKY Headset] headset (state:%d, ex:%d, key_code:%d)\n", current_headset_state, ex_headset_state, key_code);
                     #if 1  // SangwonLee 100629 
            		report_headset_switch(hs->ipdev, key, current_headset_state);
                     #else
...
#endif
              ex_headset_state = current_headset_state;
#endif
#endif
		break;
...
#if 1		/* Pantech - SKY :: Need RPC Server !!! // GINGERBREAD Updated  */
	rc = hs_rpc_cb_init();
	if (rc) {
...
{
#ifdef POWER_WAKEUP_WAIT_1SEC
	wake_lock_init(&power_wae_lock, WAKE_LOCK_SUSPEND, "power_wae_lock");
#endif
