...
#define NV_VERS_6_1   0x00060001 /* 0x00060001 */
...
int msm_nv_rpc_connect(void)
{
	...
	/* Initialize rpc ids */
	if (msm_nv_init_rpc_ids(NV_VERS_6_1)) {
		printk(KERN_ERR "%s: rpc ids(0x%x) initialization failed\n",
			__func__, NV_VERS_6_1);
		return -ENODATA;
	}
	...
	if (IS_ERR(nv_ep)) {
		...
		/* Initialize rpc ids version 0x00020001 */
		if (msm_nv_init_rpc_ids(NV_VERS_2_1)) {
			printk(KERN_ERR "%s: rpc ids(0x%x) initialization failed\n",
				__func__, NV_VERS_2_1);
			return -ENODATA;
		}
