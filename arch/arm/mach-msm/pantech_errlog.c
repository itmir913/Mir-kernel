 ...
 * ******************************************************************/
//extern void force_lcd_screen_on(bool clock_on);
...
static void drawglyph(unsigned char *base, int pixels, unsigned short paint,
                      unsigned stride, unsigned *glyph, int bpp)
{
    unsigned x, y, data;
    stride -= 5;
...
static void display_log(const char *str, int size)
{
	...
	if(bpp == 2) //RGB565
	{
		for(k = 0; k < fb_num; k++ )
		{
			...
			base  = (unsigned short *)(((unsigned char *)info->screen_base) + fb_width * fb_height * bpp * k);
			t_cx = cx; t_cy = cy;
...
void pantech_errlog_display(bool bmArm, bool do_panic)
{
	
	if(atomic_read(&is_displayed) == 0) {
		atomic_set(&is_displayed, 1);

		display_log("[ERROR LOG]\n\n", strlen("[ERROR LOG]\n\n"));
		display_log(pDisplayBuffer, DisplayLogIdx);
		display_log("\n\n    Press End key for reboot\n\n", strlen("\n\n    Press End key for reboot\n\n"));
#if 0
		if(bmArm)
			force_lcd_screen_on(false);
		else
			force_lcd_screen_on(true);
...
EXPORT_SYMBOL(pantech_errlog_display);
...
//	if(cmd == SYS_RESET_REASON_ANDROID) {
		...
		if(copy_from_user(&pErrorLog[0], buf + 6, len))
			return -EFAULT;
		LogIdx = len;
