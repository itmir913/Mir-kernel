...
#endif
#ifdef CONFIG_SKY_SND_MAX8981 //CONFIG_SKY_SND_MAX97001 97001 -> 8981 amp test 101214
#include "sky_snd_ext_amp_max8981.h" //"sky_snd_ext_amp_max97001.h" 97001 -> 8981 amp test 101214
int sub_device = SND_DEVICE_DEFAULT;
...
int g_call_status=0;
#endif//CONFIG_SKY_SND_MAX8981
#ifdef FEATURE_SKY_SND_VOIP  // SangwonLee 101013
static int voip_mvs_enable=0;
#endif
static long snd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SND_SET_CALL_MODE:
		...
		dmsg.args.client_data = 0;
              ...
              if(voip_mvs_enable!=(int) dmsg.args.ear_mute)     {
        		MM_INFO("snd_set_call_mode %d %d %d\n", dev.device,
        				dev.ear_mute, dev.mic_mute);

        		rc = msm_rpc_call(snd->ept,
        			SND_SET_DEVICE_PROC,
        			&dmsg, sizeof(dmsg), 5 * HZ);
                     voip_mvs_enable = (int) dmsg.args.ear_mute;
              }
            break;
#endif
	case SND_SET_DEVICE:
...
#ifdef CONFIG_EF31S_32K_BOARD
		if (loopback_test) {
			MM_ERR("snd_set_device: Loopback test enabled!!!!\n");
			dev.ear_mute = 0;
			dev.mic_mute = 0;
		}
#endif
#ifdef CONFIG_SKY_SND_MAX8981 //CONFIG_SKY_SND_MAX97001 97001 -> 8981 amp test 101214
		printk(KERN_ERR "##### dev.device=[%d]\n", dev.device);
		printk(KERN_ERR "##### cpu_to_be32(dev.device)=[%d]\n",
				cpu_to_be32(dev.device));
		if (dev.device == SND_DEVICE_HEADSET_AND_SPEAKER)   // SangwonLee 100513
		{
			//printk(KERN_ERR "##### SND_DEVICE_HEADSET_AND_SPEAKER -> SND_DEVICE_CURRENT\n");
			#ifdef FEATURE_SKY_SND_DUAL_PATH_DEFAULT_SPEAKER
			dmsg.args.device = cpu_to_be32(SND_DEVICE_SPEAKER_PHONE);
            #else
            dmsg.args.device = cpu_to_be32(SND_DEVICE_STEREO_HEADSET);
            #endif
		}
		else
		{
		...
		MM_INFO("snd_set_device %d %d %d\n", dev.device,
				dev.ear_mute, dev.mic_mute);
#ifdef CONFIG_SKY_SND_MAX8981 //CONFIG_SKY_SND_MAX97001 97001 -> 8981 amp test 101214
		printk(KERN_ERR "##### snd_set_device %d %d %d\n", dev.device,
				dev.ear_mute, dev.mic_mute);
#endif//CONFIG_SKY_SND_MAX8981
		...
		rc = msm_rpc_call(snd->ept,
			...
			&dmsg, sizeof(dmsg), 5 * HZ);
#ifdef CONFIG_SKY_SND_MAX8981 //CONFIG_SKY_SND_MAX97001 97001 -> 8981 amp test 101214
		sub_device = dev.device;
              snd_extamp_api_Snd_SetDevice(sub_device, dev.ear_mute, dev.mic_mute);
#endif//CONFIG_SKY_SND_MAX8981
		break;
...
#ifdef CONFIG_SKY_SND_MAX8981 //CONFIG_SKY_SND_MAX97001 97001 -> 8981 amp test 101214
	case SND_SKY_COMMON_PARAM:
              if (get_user(status, (uint32_t __user *) arg)) {
			rc = -EFAULT;
			break;
              }
              g_call_status=status;
              //MM_INFO("[SND] AUDIO_SKY_COMMON_PARAM %d\n", g_call_status);
		break;
#endif//CONFIG_SKY_SND_MAX8981
	default:
