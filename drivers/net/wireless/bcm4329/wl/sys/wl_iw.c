...
#ifdef CUSTOMER_HW_PT	/* 20110715_roam_patch */
static int
wl_iw_set_roam_status(
	struct net_device *dev,
	struct iw_request_info *info,
	union iwreq_data *wrqu,
	char *extra
)
{
	...
	WL_ERROR(("%s: set dhd_roam_disable = %d\n", __FUNCTION__, dhd_roam_disable));

	error = dev_wlc_intvar_set(dev, "roam_off", dhd_roam_disable);
...
)
{
	...
	if (dwrq->length && extra) {
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
			...
			ret = wl_iw_set_roam_status(dev, info, (union iwreq_data *)dwrq, extra);
#endif
