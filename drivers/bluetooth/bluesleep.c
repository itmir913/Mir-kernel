...
#define BLUEZ_BRCM_H4_LPM_SUPPORT

/* DS2@dscheon. 	*/
#define FROYO_BT_INT_BUG_FIXED

#ifdef BLUEZ_BRCM_H4_LPM_SUPPORT
...
//	BT_DBG("hostwake line change");
	printk(KERN_INFO"bluesleep_hostwake_task \n");	
...
static void bluesleep_tx_timer_expire(unsigned long data)
{
	...
	if (!test_bit(BT_TXDATA, &flags)) {
		...
		if((bsi->uport != NULL)&&(msm_hs_tx_empty(bsi->uport))){	/* 110125 seoutek jjh - reset problem*/
			clear_bit(BT_TXDATA, &flags);
		}
#endif
	}

#ifndef BLUEZ_BRCM_H4_LPM_SUPPORT
...
#endif
#ifndef BLUEZ_BRCM_H4_LPM_SUPPORT
	gpio_clear_detect_status(bsi->host_wake_irq);	// Needless Code
#endif
...
static int bluesleep_start(void)
{
	...
	gpio_set_value(bsi->ext_wake, 0);
#ifdef FROYO_BT_INT_BUG_FIXED
	printk(KERN_ERR "%s: 83 pin status is %d\n", __func__, gpio_get_value(bsi->host_wake));
