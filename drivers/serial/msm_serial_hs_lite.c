...
#define USER_BUILD_RX_EN_TEMP		// 2010.07.07 For RX enable in case of an User Build by dscheon
#endif
...
void msm_hsl_console_set_terminal_onoff(uint32_t onoff)
{
	...
	printk(KERN_INFO "msm_hsl_console_set_terminal_onoff: onoff=%d, index=%d\n",
	       onoff, (MSM_HSL_CONSOLE->index));

	port = get_port_from_line(MSM_HSL_CONSOLE->index);
  msm_hsl_port = UART_TO_MSM(port);

	if (onoff == 1) {
		b_terminal_onoff = 1;
		msm_hsl_write (port, UARTDM_CR_RX_EN_BMSK, UARTDM_CR_ADDR);
	} else {
		b_terminal_onoff = 0;
#ifdef USER_BUILD_RX_EN_TEMP
		// Test Func by dscheon
		msm_hsl_write (port, UARTDM_CR_RX_EN_BMSK, UARTDM_CR_ADDR);
#else
		msm_hsl_write (port, UARTDM_CR_RX_DISABLE_BMSK, UARTDM_CR_ADDR);
#endif
	}
}
//#else
...
//{
//}
//#endif // #ifdef FEATURE_AARM_RELEASE_MODE
