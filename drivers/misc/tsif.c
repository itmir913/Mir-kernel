...
//아래 Register는 디버깅 필요시 변경해야함(MSM 별로 다를수 있음)
...
//#define FEATURE_TSIF_DUMP

#ifdef FEATURE_TSIF_DUMP
#define DUMP_SIZE_MAX 1504*1000
char dump_data[DUMP_SIZE_MAX];
#endif /* FEATURE_TSIF_DUMP */
...
#ifdef FEATURE_TSIF_DEBUG_MSG
	pr_info("[%s] ctl [0x%8x] time_limit[%d], base[%08x]\n", __func__, ctl, tsif_device->time_limit, tsif_device->base);
#endif
...
#ifdef FEATURE_TSIF_DEBUG_MSG
			int **data_addr = tsif_device->data_buffer + data_offset;
