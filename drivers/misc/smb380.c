...
int smb380_set_offset_eeprom_dummy(unsigned char xyz, unsigned short offset) 
{
...
unsigned char  smb380_read_id(unsigned char *output_data) 
{
	int comres=0;
	unsigned char data;
	...
	p_smb380->dev_addr = SMB380_I2C_ADDR;										/* preset SM380 I2C_addr */
	comres += p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_CHIP_ID__REG, &data, 1);	/* read Chip Id */
	output_data[0] = (unsigned char)SMB380_GET_BITSLICE(data, SMB380_CHIP_ID);						/* get bitslice */
