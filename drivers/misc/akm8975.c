...
#define SKY_MODIFY_FOR_EF31S		//101209 Forever ... poring to EF31S..
#endif 

#define AKM8975_DEBUG		1
#define AKM8975_DEBUG_MSG	1

#ifdef SKY_MODIFY_FOR_EF31S
...
static int AKECS_GetData(char *rbuf, int size)
{
...
#ifdef CONFIG_EF31S_32K_BOARD  // 2011.01.06 Sometimes interrupt is diabled by unknown reason.
							     // temporary solution..
...
#ifdef CONFIG_EF31S_32K_BOARD    // 2011.01.27 forever ... 
								// AKECS_GetData() called before data ready is set..
								// it is not a critical error ...copy previous data and continue waiting ...
	...
	return 0;
		...
		if(failure_count){
			printk(KERN_ERR
				       "AKM8975 AKECS_GetData: [%d ]failure and recover it \n",
				       failure_count);
		}
	}
#endif
		   ...
		   unsigned long arg)
{
	...
	case ECS_IOCTL_GET_DELAY:
		...
		delay = akmd_delay;
		break;
		case ECS_IOCTL_CHECK_DEVICE:
			AKMDBG("ECS_IOCTL_CHECK_DEVICE ");
			if (AKECS_CheckDevice() < 0) {
				AKMDBG("ERROR : FACTORY CHECK");
				return -EFAULT;
			}
			break;
	default:
...
//ak8975 devices is add as i2c devices instead of platform devices..
// thus we can't receive platform_data from kernel..and actually platform_data is not used at this driver..
#ifdef SKY_MODIFY_FOR_EF31S
	...
	/* IRQ */
//ak8975 devices is add as i2c devices instead of platform devices..
// thus we can't receive platform_data from kernel..and we should add isr at this point 
// some errors in interrupt...chaeck it again..and we shoule interrupt  enable..
#ifdef SKY_MODIFY_FOR_EF31S
{
#define AK8975_PS_INT_N			88		// interrupt gpio
#define AK8975_PS_IRQ				gpio_to_irq(AK8975_PS_INT_N)

		gpio_tlmm_config(GPIO_CFG(AK8975_PS_INT_N, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
		gpio_request(AK8975_PS_INT_N, "akm8975_DRDY");
		client->irq = gpio_to_irq(AK8975_PS_INT_N);
		err = request_irq(client->irq, akm8975_interrupt, IRQ_TYPE_EDGE_RISING,
					  "akm8975_DRDY", akm);
		printk(KERN_ERR "AKM8975 akm8975_probe: request irq  err=%d client->irq=%d\n",err,client->irq);
}
#else
	err = request_irq(client->irq, akm8975_interrupt, IRQ_TYPE_EDGE_RISING,
					  "akm8975_DRDY", akm);
