...
#define dbg(fmt, args...)   printk("[SKY_LS] " fmt, ##args)
#else
#define dbg(fmt, args...)
#endif
...
#define SENSOR_INPUTDEV_NAME	"light"
...
static void sensor_set_enable(struct device *dev, int enable)
{
	...
	if (enable) {                   /* enable if state will be changed */
		if (!atomic_cmpxchg(&sensordata->enable, 0, 1)) {
			apds9900_control_enable(E_ACTIVE_LIGHT, 1);
		...
		if (atomic_cmpxchg(&sensordata->enable, 1, 0)) {
			...
			apds9900_control_enable(E_ACTIVE_LIGHT, 0);
		}
	}
...
static void sensor_work_func(struct work_struct *work)
{
	...
	if(input_pdev == NULL) {
		...
		input_report_abs(input_pdev, ABS_X, val.x);
		input_report_abs(input_pdev, ABS_Y, val.y);
		input_report_abs(input_pdev, ABS_Z, val.z);
...
static int sensor_probe(struct platform_device *pdev)
{
	...
	// apds9900_init(APDS9900_TYPE_LIGHT);
