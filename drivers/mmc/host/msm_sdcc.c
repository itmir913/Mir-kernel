...
#define GB_6120_SDCC_ISSUE //kychoi 110218 wifi mmc1: crc error ¼öÁ¤
// #define GB_6120_WIFI_SUSPEND_BUG //kychoi 110302 gingerbread wifi didn't suspend bug //kychoi 110310

#ifdef GB_6120_SDCC_ISSUE
static struct wake_lock sdcc_idle_lock;
#endif
...
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock) {
		...
		if (!host->clks_on) {
			...
			clk_enable(host->clk);
			host->clks_on = 1;
//kychoi 110310  MMC_PM_WAKE_SDIO_IRQ add
...
static DEVICE_ATTR(polling, S_IRUG | S_IWUSR,
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (plat->status_irq) {
		...
		mmc->pm_flags |= MMC_PM_IGNORE_PM_NOTIFY;  //kychoi 110310 add
	} else if (!plat->status)
...
#if 0 //kychoi 110221 tempt test //kychoi 110310 GB original code
	if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO)
	{
		pm_runtime_get_noresume(dev);
		rc = mmc_suspend_host(mmc);
//		mmc->suspend_task = current;	// GB original - 20110301
		...
		pm_runtime_put_noidle(dev);
#endif //temp test
#ifdef GB_6120_WIFI_SUSPEND_BUG
	    if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO)
	    {
           if (!rc) {
			/*
...
#if 0 //kychoi 110221 temp test //kychoi 110310 GB original code
		if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO) {
				mmc_resume_host(mmc);
		}
#else
...
#endif
#ifdef GB_6120_SDCC_ISSUE
    wake_lock_init(&sdcc_idle_lock, WAKE_LOCK_IDLE, "sdcc_idle_wake_lock");
#endif
