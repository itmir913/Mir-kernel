...
#if defined(CONFIG_HSUSB_PANTECH_OBEX) || defined(FEATURE_ANDROID_PANTECH_USB_SKY)
#define SKY_VENDOR_ID 0x10A9
#define SKY_PRODUCT_ID 0x1104
#endif
...
#if 1//pz1716@ds1 defined(FEATURE_AARM_RELEASE_MODE)
static int android_pantech_set_serial_number(void)
{
  struct msm_rpc_client *client;
  static char output[14];
  static uint32_t out_len = sizeof(output);//0;
  static char input = 1;
...
//    return 0;
  ...
  if(pantech_serial_number == NULL) {
    pantech_serial_number = kmalloc(MAX_SERIAL_LEN, GFP_KERNEL);
  }

  memset(pantech_serial_number, 0x00, MAX_SERIAL_LEN);

  arg.event = 20;
  arg.cb_func = NULL;//0;
  arg.handle = 1;//(void*)1;//MSECTOR->BSN
  arg.in_len = 1;
  arg.input = &input;
  arg.out_len_valid = 1;
  arg.output_valid = 14;//1;
  arg.output_size = 14;
  ...
  if(*ret.out_len <= 0)
  {
//    kfree(pantech_serial_number);
//    pantech_serial_number = NULL;
...
//    return 0;
  ...
  memcpy(pantech_serial_number + strlen(SYS_MODEL_NAME), ret.output, *ret.out_len);
  oem_rapi_client_close();
...
#if defined(CONFIG_HSUSB_PANTECH_OBEX) || defined(FEATURE_ANDROID_PANTECH_USB_SKY)
		if(device_desc.idVendor == SKY_VENDOR_ID){
			pantech_serial_number = NULL;
			android_pantech_set_serial_number();
			if(pantech_serial_number == NULL){
