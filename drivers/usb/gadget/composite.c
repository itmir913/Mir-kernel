...
#ifdef FEATURE_SKY_USB_STATUS_SUPPLEMENT
...
#if defined(CONFIG_HSUSB_PANTECH_OBEX) || defined(FEATURE_ANDROID_PANTECH_USB_SKY)
			if (cdev->config == NULL)
				return -1;
#else
...
#ifdef FEATURE_ANDROID_PANTECH_USB_QC_FIX_BUG
extern enum chg_type pantech_get_otg_chg_type(void);
#ifdef FEATURE_SKY_USB_STATUS_SUPPLEMENT
unsigned char mute_switch_status=0;
extern unsigned char adb_enable_staus;
void composite_switch_work_sky(struct work_struct *data)
{
	struct usb_composite_dev	*cdev =
		container_of(data, struct usb_composite_dev, switch_work_sky);

	struct usb_configuration *config = cdev->config;

	if (config)
	{
		printk(KERN_INFO "[###### PANTECH SKY ######] switch....re set");		
		switch_set_state(&cdev->sdev, config->bConfigurationValue);
	}
	else
	{
		printk(KERN_INFO "[###### PANTECH SKY ######] switch....re clear");		
		switch_set_state(&cdev->sdev, 0);
	}
}

static void composite_switch_work_timer(unsigned long handle)
{
	struct usb_composite_dev	*cdev = (struct usb_composite_dev *) handle;;

	printk(KERN_INFO "[###### PANTECH SKY ######] switch....[mute_switch_status:%d]",mute_switch_status);

	if(mute_switch_status);
	{
		schedule_work(&cdev->switch_work_sky);
		mute_switch_status=0;
	}
}
#endif
#endif/*FEATURE_ANDROID_PANTECH_USB_QC_FIX_BUG*/
...
#ifdef FEATURE_ANDROID_PANTECH_USB_QC_FIX_BUG
    if (cdev->mute_switch && pantech_get_otg_chg_type() == USB_CHG_TYPE__SDP)
#else
	if (cdev->mute_switch)
#endif/*FEATURE_ANDROID_PANTECH_USB_QC_FIX_BUG*/
	{		
		cdev->mute_switch = 0;
#ifdef FEATURE_SKY_USB_STATUS_SUPPLEMENT
		mute_switch_status = 1;
		setup_timer(&cdev->timer, composite_switch_work_timer, (unsigned long) cdev);
		mod_timer(&cdev->timer, jiffies + (HZ/50));  
#endif
	}
	else
		schedule_work(&cdev->switch_work);
...
composite_switch_work(struct work_struct *data)
{
	...
	if (config)
	{
		printk(KERN_INFO "[###### PANTECH SKY ######] composite_switch_work....set");				
		switch_set_state(&cdev->sdev, config->bConfigurationValue);
	}
	else
	{
		printk(KERN_INFO "[###### PANTECH SKY ######] composite_switch_work....clear");				
		switch_set_state(&cdev->sdev, 0);
	}
}
...
#ifdef FEATURE_SKY_USB_STATUS_SUPPLEMENT
	INIT_WORK(&cdev->switch_work_sky, composite_switch_work_sky);
#endif
