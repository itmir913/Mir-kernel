...
#if defined(FEATURE_ANDROID_PANTECH_USB_SKY)
...
static int obex_open(struct inode *ip, struct file *fp)
{
  ...
  PRINTK_OBEX("obex_open (%x)(%d)\n", (uint32_t)dev, atomic_read(&dev->open_excl));
	...
	dev->read_count = 0;
	if (dev->read_req) {
		PRINTK_OBEX("obex_open : unreaded req(%d)\n",dev->read_count); 
		req_put(dev, &dev->rx_idle, dev->read_req);
		dev->read_req = 0;
	}
...
int pantech_obex_function_init(void)
{
  struct obex_dev *dev;
  int ret;
...
err1:
  misc_deregister(&obex_device);
  kfree(dev);
  printk(KERN_ERR "obex gadget driver failed to initialize\n");
  return ret;
}
...
static int pantech_obex_bind_config(struct usb_configuration *c)
{
  printk(KERN_ERR "pantech_obex_bind_config\n");
	pantech_obex_function_init();
	(void)pantech_obex_function_add((struct usb_composite_dev *)_obex_dev, c);	
	return 0;
}
...
static int __init pantech_obex_init(void)
{
  printk(KERN_INFO "pantech_obex init\n");
  android_register_function(&pantech_obex_function);
  return 0;
}
module_init(pantech_obex_init);
