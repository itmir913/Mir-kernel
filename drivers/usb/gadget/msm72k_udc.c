...
extern usb_test_cmd_type loopback_usb_test_cmd;
#endif
...
static void handle_setup(struct usb_info *ui)
{
	...
	if(((ctl.bRequestType & USB_TYPE_MASK) != USB_TYPE_STANDARD)
     && ((ctl.bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR)
		 && ((ctl.bRequestType & USB_RECIP_MASK) == USB_RECIP_DEVICE)
		 && (ctl.bRequest == SKT_USB_CMD_LOOPBACK_APP || ctl.bRequest == SKT_USB_CMD_PORT_CONTROL) ) {

		if(ui->usb_state == USB_STATE_CONFIGURED) {

			if(ctl.bRequest == SKT_USB_CMD_LOOPBACK_APP) {
				loopback_usb_test_cmd.run = ctl.wValue;
				loopback_usb_test_cmd.open = 0;
				loopback_usb_test_cmd.type = 0xff;

				if(loopback_usb_test_cmd.run == 0x0000) {
					obex_loopback_close();
				} else if(loopback_usb_test_cmd.run == 0x0001){
					obex_loopback_open();
				}

			} else if(ctl.bRequest == SKT_USB_CMD_PORT_CONTROL) {
				loopback_usb_test_cmd.open = ctl.wValue;
				loopback_usb_test_cmd.type = ctl.wIndex;
			}
		}
...
#if defined(CONFIG_HSUSB_PANTECH_USB_TEST)
	pantech_usb_test_bind_loopback(&ui->ep0in.ep);
#endif
