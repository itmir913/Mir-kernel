...
#if defined(CONFIG_HSUSB_PANTECH_USB_TEST)
  if((port->port_num == 0)//ttyGS0 
		&& (pantech_usb_test_get_run() == 1)
    &&  (pantech_usb_test_get_open() == 1)
    && (pantech_usb_test_get_port_type() == 0))
  {
    //printk("gs_loopback_read size[%d] = count[%d]\n", req->length, req->actual);

    if(req->status == 0) {
      if(req->actual == 0) {
        pantech_usb_test_send_loopback((void*)ep->driver_data, port->port_usb->in);
		...
		usb_ep_queue(port->port_usb->out, req, GFP_ATOMIC);
    return;
  }
#endif
