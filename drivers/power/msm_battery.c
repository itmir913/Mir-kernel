...
#define FEATURE_SKY_NO_CHARGER_RPC
#define FEATURE_SKY_BATT
#define FEATURE_SKY_VBATT_RPC 		// battery Level(%) , Volt(mV) and charger state (Type) read RPC. Inserted by dscheon[DS2]
#endif

#define FEATURE_SKY_BATT_RESUME_2	
...
#ifdef FEATURE_SKY_VBATT_RPC
#define SKY_VBATT_READ_PROC 30
#endif
...
#ifndef FEATURE_SKY_CHG_LOGO//pz1716
#define FEATURE_SKY_BATT_ID_NO_CHECK	 // EF31S_EF32K는 H/W회로에서 BATT_ID를 CHECK할수 없음.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
#endif
...
#ifndef FEATURE_SKY_NO_CHARGER_RPC
...
#ifdef FEATURE_SKY_BATT	
	s32 charging_current; /* jake*/
	s32 current_chg_status;
	s32 vbatt_id;

	s32 batt_transistor_status;
	s32 chg_transistor_status;
#ifdef FEATURE_SKY_OVP_FAULT_DETECT	
	s32 OVP_status;		
#endif		
#endif // FEATURE_SKY_BATT	

#ifdef FEATURE_SKY_BATT_RESUME_2 
	/* Changed early_suspend -> normal suspend by dscheon [DS2] 2010.09.27 */
...
static struct msm_battery_info msm_batt_info = {
	...
	.vbatt_id = 0,
	.batt_transistor_status = 0,
	.chg_transistor_status = 0,
#ifdef FEATURE_SKY_OVP_FAULT_DETECT	
	.OVP_status = 0,
#endif		
#endif // FEATURE_SKY_BATT	
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		break;
#ifdef FEATURE_SKY_BATT	
    // Battery Test Menu Display
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = msm_batt_info.charging_current;
		break; /* jake */
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		val->intval = msm_batt_info.current_chg_status;
		break;
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = msm_batt_info.battery_temp;
		break;
	case POWER_SUPPLY_PROP_TEMP_AMBIENT:
		val->intval = msm_batt_info.vbatt_id;
		break;
	case POWER_SUPPLY_PROP_CHARGE_AVG:
		val->intval = msm_batt_info.batt_transistor_status;
		break;		
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		val->intval = msm_batt_info.chg_transistor_status;
		break;		
#endif  // FEATURE_SKY_BATT
...
#ifndef CONFIG_BATTERY_MSM_FAKE
#ifdef FEATURE_SKY_VBATT_RPC
struct msm_batt_sky_get_batt_chg_req_data {
	u32 more_data;
};

struct msm_batt_sky_get_batt_chg_rsp_data {
	u32 	more_data;
    ...
    u32       chg_current_status;
    u32       chg_temperature;
    u32       vbatt_id;
    u32       batt_chg_tr_status;
#ifdef FEATURE_SKY_OVP_FAULT_DETECT	
    u32	  OVP_status;
#endif	
};

static int msm_batt_sky_get_batt_chg_arg_func(struct msm_rpc_client *batt_client,
				       void *buf, void *data)
{
	struct msm_batt_sky_get_batt_chg_req_data *sky_batt_read_req =
		(struct msm_batt_sky_get_batt_chg_req_data *)data;
	u32 *req = (u32 *)buf;
	int size = 0;

	*req = cpu_to_be32(sky_batt_read_req->more_data);
	size += sizeof(u32);

	return size;
}

static int msm_batt_sky_get_batt_chg_ret_func(struct msm_rpc_client *batt_client,
				       void *buf, void *data)
{
	struct msm_batt_sky_get_batt_chg_rsp_data *data_ptr, *buf_ptr;

	data_ptr = (struct msm_batt_sky_get_batt_chg_rsp_data *)data;
	buf_ptr = (struct msm_batt_sky_get_batt_chg_rsp_data *)buf;

	if( be32_to_cpu(buf_ptr->more_data) )
	{
		data_ptr->battery_level = be32_to_cpu(buf_ptr->battery_level);
		data_ptr->battery_voltage = be32_to_cpu(buf_ptr->battery_voltage);
		data_ptr->charger_status = be32_to_cpu(buf_ptr->charger_status);
        data_ptr->charging_current = be32_to_cpu(buf_ptr->charging_current); /* jake */
        data_ptr->chg_current_status = be32_to_cpu(buf_ptr->chg_current_status);
        data_ptr->chg_temperature = be32_to_cpu(buf_ptr->chg_temperature);
        data_ptr->vbatt_id = be32_to_cpu(buf_ptr->vbatt_id);
        data_ptr->batt_chg_tr_status = be32_to_cpu(buf_ptr->batt_chg_tr_status);
#ifdef FEATURE_SKY_OVP_FAULT_DETECT
        data_ptr->OVP_status = be32_to_cpu(buf_ptr->OVP_status);
#endif		
	}
	else
	{
		printk(KERN_INFO "%s():No more data in vbatt rsp buffers\n",
				__func__);
	}
...
#ifdef FEATURE_SKY_OVP_FAULT_DETECT
static int msm_batt_sky_get_batt_chg_status(u32 *batt_level,u32 *batt_volt, u32 *batt_charging,
    u32 *charging_current, u32 *chg_current_status, u32 *chg_temperature, u32 *vbatt_id, u32 *batt_chg_tr_status, u32 *OVP_status)
#else
static int msm_batt_sky_get_batt_chg_status(u32 *batt_level,u32 *batt_volt, u32 *batt_charging,
    u32 *charging_current, u32 *chg_current_status, u32 *chg_temperature, u32 *vbatt_id, u32 *batt_chg_tr_status)
#endif    
{
	struct msm_batt_sky_get_batt_chg_req_data req;
	struct msm_batt_sky_get_batt_chg_rsp_data rsp;
	int rc;
	...
	rc = msm_rpc_client_req(msm_batt_info.batt_client,
			SKY_VBATT_READ_PROC,
				msm_batt_sky_get_batt_chg_arg_func, &req,
				msm_batt_sky_get_batt_chg_ret_func, &rsp,
	...
	*batt_level = rsp.battery_level;
	*batt_volt = rsp.battery_voltage;
	*batt_charging = rsp.charger_status;
	*charging_current = rsp.charging_current;
	*chg_current_status = rsp.chg_current_status;
	*chg_temperature = rsp.chg_temperature;
	*vbatt_id = rsp.vbatt_id;
	*batt_chg_tr_status = rsp.batt_chg_tr_status;	
#ifdef FEATURE_SKY_OVP_FAULT_DETECT	
	*OVP_status = rsp.OVP_status;
#endif	
	return 0;
}

static uint32_t fake_usb_flag;
void msm_batt_sky_fake_usb_info(uint32_t onoff)
{
	fake_usb_flag = onoff;	
}
EXPORT_SYMBOL(msm_batt_sky_fake_usb_info);

static void msm_batt_sky_update_psy_status(void)
{
	u32 batt_permil = 0;  
	u32 batt_volt = 0;
	u32 batt_charging = 0;
	u32 charging_current = 0;
	u32 chg_current_status = 0;
	u32 chg_temperature = 0;
	u32 vbatt_id = 0;
	u32 batt_chg_tr_status = 0;
	u32 OVP_status =0;
#ifdef FEATURE_SKY_OVP_FAULT_DETECT	
	msm_batt_sky_get_batt_chg_status(&batt_permil, &batt_volt, &batt_charging, &charging_current,
        &chg_current_status, &chg_temperature, &vbatt_id, &batt_chg_tr_status, &OVP_status);
#else	
	msm_batt_sky_get_batt_chg_status(&batt_permil, &batt_volt, &batt_charging, &charging_current,
        &chg_current_status, &chg_temperature, &vbatt_id, &batt_chg_tr_status);
#endif

	msm_batt_info.batt_valid = 1;
	msm_batt_info.battery_voltage = batt_volt;
	msm_batt_info.batt_capacity = msm_batt_info.calculate_capacity(batt_permil);
	msm_batt_info.charging_current = charging_current;
	msm_batt_info.current_chg_status = chg_current_status;
	msm_batt_info.battery_temp = chg_temperature;
	msm_batt_info.vbatt_id = vbatt_id;


	/* Inserted by dscheon. For TR status Display	*/
	msm_batt_info.chg_transistor_status = batt_chg_tr_status & 0x0F;
	msm_batt_info.batt_transistor_status = (batt_chg_tr_status >> 4) & 0x0F;

	/* Inserted by dscheon. For IDLE Status vbatt id display error	*/
#ifndef FEATURE_SKY_BATT_ID_NO_CHECK
	if (batt_charging == 0 && vbatt_id == (CHG_MIN_VALID_BATT_ID + 10))
		 msm_batt_info.vbatt_id = 0;	
#endif
	// added by jwcha100803@DS2 for tethering debug -->
	if (fake_usb_flag && batt_charging == 1) {
		printk(KERN_INFO "fake USB charging info (for tethering debug)\n");
		batt_charging = 2;
	}
	// added by jwcha100803@DS2 for tethering debug <--

#ifdef FEATURE_SKY_OVP_FAULT_DETECT	
	msm_batt_info.OVP_status = OVP_status;
#endif

	printk(KERN_INFO "batt_level(permil) = %u batt_volt = %u batt_charging = %u\n", batt_permil,batt_volt,batt_charging );
	printk(KERN_INFO "charging_current = %u, chg_current_status = %u, chg_temp = %u\n", charging_current, chg_current_status, chg_temperature);
#ifdef FEATURE_SKY_OVP_FAULT_DETECT	
	printk(KERN_INFO "vbatt_id = %u, Ovp status = %d \n", vbatt_id, OVP_status);
#endif
#if (1)
	/* Copy from EF12S	*/
	if( msm_batt_info.batt_status == POWER_SUPPLY_STATUS_FULL )
	{
		static int once_more_flag = 0;
		if( batt_charging == 1 ||batt_charging == 2) 	// Charging
		{
			printk(KERN_INFO "Display Full Charging : batt_capacity %d -> 100\n", msm_batt_info.batt_capacity);
			msm_batt_info.batt_capacity = 100;
			once_more_flag = 1;
		}
		else 	// Discharging		
		{
			if( once_more_flag == 1 ) // Charging -> Discharging : Complementary measure : Rapidly Decrease battery level	: First display : 100%
			{
				printk(KERN_INFO "Display once more batt_capacity 100%\n");
				msm_batt_info.batt_capacity = 100;
				once_more_flag++;
			}
			else if( once_more_flag >= 2 ) // Secondly display : 99%
			{
				if( msm_batt_info.batt_capacity < 99 ) // 99% Over in case : Intact display 
				{
					printk(KERN_INFO "Display once more batt_capacity 99%\n");
					msm_batt_info.batt_capacity = 99;
				}
				once_more_flag = 0; // Third display : usual display
			}
		}
	}
	...
	//				if( batt_permil >=  990 ) 	// 99% above ->  Currently Full Charging : 99%. ~ 100 %
			...
			//An actual state : Not use for APP. msm_batt_info.battery_level value
			if( msm_batt_info.batt_capacity >= 100 )
				msm_batt_info.battery_level = BATTERY_LEVEL_FULL;
			else if( msm_batt_info.batt_capacity > 15 )
				msm_batt_info.battery_level = BATTERY_LEVEL_GOOD;
			else if( msm_batt_info.batt_capacity > 0 )
				msm_batt_info.battery_level = BATTERY_LEVEL_WEAK;
			else
				msm_batt_info.battery_level = BATTERY_LEVEL_DEAD;
	...
	printk(KERN_INFO "Previous charger valid status = %u"
			...
			msm_batt_info.charger_valid, batt_charging);
	
	/*	USB or AC or Discharging State Indication. Inserted by dscheon[DS2]	*/
	if (msm_batt_info.charger_valid != batt_charging)
	{
		msm_batt_info.charger_valid = batt_charging;


		switch (msm_batt_info.charger_valid)
		{
			case 0:
				{
			...
			case 2:
				{
					msm_batt_info.current_chg_source &= ~AC_CHG;
					msm_batt_info.current_chg_source |= USB_CHG;
					msm_batt_info.charger_type = CHARGER_TYPE_USB_PC;
					msm_batt_info.charger_status = CHARGER_STATUS_GOOD;
					power_supply_changed(&msm_psy_usb);					
				}
				break;
...
#else 	/*	#ifdef FEATURE_SKY_VBATT_RPC		*/
...
#ifdef FEATURE_SKY_BATT
#ifdef FEATURE_SKY_CHG_LOGO//pz1716
extern uint32_t sky_sys_rst_Get_ChargingMode(void);
#endif
int msm_batt_suspend(struct platform_device *pdev, pm_message_t state)
{
	int rc;

	pr_debug("%s: enter\n", __func__);
#ifdef FEATURE_SKY_CHG_LOGO//pz1716
	pr_err("%s(): sleep limit %d\n", __func__, sky_sys_rst_Get_ChargingMode());

	if(sky_sys_rst_Get_ChargingMode()) //SKY_CHG_OFFLINE_CHARGING_MODE
		return  -EBUSY;
#endif
	if (msm_batt_info.batt_handle != INVALID_BATT_HANDLE) 
	{
#ifdef FEATURE_SKY_BATT		
		/* Early suspend state and under 15%. Call back function. Inserted by dscheon [DS2] */
		rc = msm_batt_modify_client(msm_batt_info.batt_handle,
				(BATTERY_LOW+150), BATTERY_VOLTAGE_BELOW_THIS_LEVEL,
...
#if defined FEATURE_SKY_BATT && ! defined(FEATURE_SKY_BATT_RESUME_2)
	msm_batt_sky_update_psy_status();	
#endif
...
#ifdef FEATURE_SKY_BATT_RESUME_2
void msm_batt_early_suspend_2(struct early_suspend *h)
{
	int rc;
...
void msm_batt_late_resume_2(struct early_suspend *h)
{
	int rc;
...
#ifndef FEATURE_SKY_BATT // fuel gauge Use. filter disabled
...
#endif	//FEATURE_SKY_NO_CHARGER_RPC
...
#endif	/*	#ifndef FEATURE_SKY_NO_CHARGER_RPC	*/
...
#ifndef FEATURE_SKY_BATT // Disabled . Because of fuel gauge using. by dscheon .
...
#ifdef FEATURE_SKY_BATT_RESUME_2
	msm_batt_info.early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	msm_batt_info.early_suspend.suspend = msm_batt_early_suspend_2;
	msm_batt_info.early_suspend.resume = msm_batt_late_resume_2;
	register_early_suspend(&msm_batt_info.early_suspend);
#endif
