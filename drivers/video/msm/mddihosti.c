...
//	mddi_host_reg_out(TA1_LEN, 0x000A);	//[BIH] mddi modify...

	/* Turn Around 2 register (= 0x0C) */
	mddi_host_reg_out(TA2_LEN, MDDI_HOST_TA2_LEN);

	/* Drive hi register (= 0x96) */
//	mddi_host_reg_out(DRIVE_HI, /*0x0096*/0x12C);	//[BIH] mddi modify... timeout.
	mddi_host_reg_out(DRIVE_HI, 0x0096);

	/* Drive lo register (= 0x32) */
//	mddi_host_reg_out(DRIVE_LO, /*0x0032*/0x640);	//[BIH] mddi modify... timeout.
	mddi_host_reg_out(DRIVE_LO, 0x0032);	//[BIH] mddi modify... timeout.
...
//pantch_lks@PS3_20110112 - ef31s_ef32k 	
void mddi_get_capability(mddi_client_capability_type *pMddiCapability)
{
#ifndef FEATURE_MDDI_DISABLE_REVERSE
	...
	memcpy((void *)pMddiCapability,
				       (void *)&mddi_client_capability_pkt,
				       sizeof(mddi_client_capability_type));
