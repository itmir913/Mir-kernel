...
static int mddi_on(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "[MDDI_SKY_LCD] mddi_clk : %d, clk : %d\n", clk_get_rate(mddi_clk),clk_rate);
...
static int mddi_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "[MDDI_SKY_LCD] mfd->fb_imgType : %d\n", mfd->fb_imgType);
...
static int __init mddi_driver_init(void)
{
	...
	printk(KERN_ERR "[MDDI_SKY_LCD] mddi_clk : %d\n", clk_get_rate(mddi_clk));
