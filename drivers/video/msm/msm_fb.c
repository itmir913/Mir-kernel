...
#define INIT_IMAGE_FILE "/logo.rle"	// "/initlogo.rle"
extern int load_565rle_image(char *filename);
#ifdef CONFIG_SKY_LCD_MODIFY
extern int sky_lcdc_display_loadingbar(int ratio);
#endif //CONFIG_SKY_LCD_MODIFY
#endif

#ifdef CONFIG_SKY_LCD_MODIFY
static int sky_loadbar_ratio = 0;
//extern void lcdc_gordon_screen_off(void);
extern void sky_lcd_screen_off(void);	//pantch_lks@PS3_20111118 - prepare ef31s_ef32k 	
#endif //CONFIG_SKY_LCD_MODIFY

#ifdef FEATURE_SKY_CHG_LOGO
extern uint32_t sky_sys_rst_Get_ChargingMode(void);
#define BATT_CHARGING_IN "/poweroff_icon_in.rle"
#endif
...
#ifdef CONFIG_SKY_MSM_FB_NO_CURSOR
...
{	
	...
	bl_lvl = value / (MAX_BACKLIGHT_BRIGHTNESS / mfd->panel_info.bl_max);
...
static int msm_fb_probe(struct platform_device *pdev)
{
...
#endif
        {
            //lcdc_gordon_screen_off(); //LeeCY.. for black screen before H/W init
           sky_lcd_screen_off();			//pantch_lks@PS3_20111118 - prepare ef31s_ef32k 
        }
#endif
	...
	printk(KERN_ERR "[MDDI_SKY_LCD] Exit %s\n", __FUNCTION__);
	return 0;
}
...
*/
	...
	default:
		...
		if (mfd->panel_power_on) {
			...
			//printk(KERN_DEBUG "%s . FB_BLANK_POWERDOWN\n", __FUNCTION__);
#ifdef	CONFIG_FB_MSM_MDDI		//pantch_lks@PS3_20110112 - ef31s_ef32k 	
			{
				struct fb_info *fbi = mfd->fbi;		
				//printk(KERN_ERR "[MDDI_SKY_LCD] msm_fb_blank_sub-1 : Clear Screen \n");
		...
		if (mfd->panel_power_on) {
			...
			//msleep(16);
	                mdelay(100);//pantch_lks@PS3_20111204 - prepare ef31s_ef32k  		
			ret = pdata->off(mfd->pdev);
			if (ret)
				mfd->panel_power_on = curr_pwr_state;
...
static struct fb_ops msm_fb_ops = {
	...
	.fb_read = NULL,
	.fb_write = NULL,
#ifndef CONFIG_SKY_MSM_FB_NO_CURSOR
	.fb_cursor = NULL,
#else
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
...
#if 1 //240??ик ????? board feature ??.. leecy
	// added by jwcha090929
	var->height = 75,	/* height of picture in mm */
	var->width = 45,	/* width of picture in mm */
#else
    ...
    if (!sky_sys_rst_is_silent_boot_mode()) {
#endif
#ifndef FEATURE_SKY_BOOT_LOGO_WITH_MDDI	// D_PS3_PZ1569_BOOT_LOGO - 20101227
	...
	MSM_FB_INFO
      ...
      {
          int x = load_565rle_image(BATT_CHARGING_IN);
	MSM_FB_DEBUG("FrameBuffer[%s]!\n",  BATT_CHARGING_IN);
     printk(KERN_DEBUG "load_565rle_image:%d %s\n", x,BATT_CHARGING_IN);
#ifdef FEATURE_SKY_BOOT_LOGO_WITH_MDDI	// D_PS3_PZ1569_BOOT_LOGO - 20101221
	...
	if (!load_565rle_image(INIT_IMAGE_FILE)) ;	/* Flip buffer */
     ...
     printk(KERN_DEBUG "load_565rle_image. loadbar ratio %d\n", sky_loadbar_ratio);

#ifndef FEATURE_SKY_LOADBAR_INCREASABLE	// D_PS2_PZ1569_SKY_LOADBAR - 20110226 - BSH
     sky_loadbar_ratio += 20;
     sky_lcdc_display_loadingbar(sky_loadbar_ratio);
#endif
