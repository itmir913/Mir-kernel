...
/*jwjeon100717@DS2*/
#ifdef CONFIG_CPU_FREQ_PERFLOCK
				   ...
				   const char *buf, size_t count)
{
  ...
  cpufreq_perflock_set_ondemand_sampling_rate(input);
#endif
...
unsigned int cpufreq_ondemand_get_sampling_rate(void)
{
  return dbs_tuners_ins.sampling_rate;
}

void cpufreq_ondemand_set_sampling_rate(unsigned int sampling_rate)
{
  mutex_lock(&dbs_mutex);
  dbs_tuners_ins.sampling_rate = sampling_rate;
  mutex_unlock(&dbs_mutex);
}
